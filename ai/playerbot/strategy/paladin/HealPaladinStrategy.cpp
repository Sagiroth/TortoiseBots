// Forward-ported from mod-playerbots Paladin/Strategy/HealPaladinStrategy.cpp
...
void HealPaladinStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(
        new TriggerNode(
            "medium mana",
            {
                NextAction("divine favor", ACTION_HIGH + 2)
            }
        )
    );
    triggers.push_back(
    ...
    triggers.push_back(
        new TriggerNode(
            ...
            {
                NextAction("holy light on party", ACTION_CRITICAL_HEAL + 5),
            }
        )
    );
    triggers.push_back(
