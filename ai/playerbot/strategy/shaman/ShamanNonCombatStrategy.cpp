// Forward-ported from mod-playerbots Shaman/Strategy/ShamanNonCombatStrategy.cpp
...
void ShamanNonCombatStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode("party member critical health", { NextAction("healing wave on party", 30.0f) }));
    triggers.push_back(new TriggerNode("party member low health", { NextAction("healing wave on party", 28.0f) }));
    triggers.push_back(new TriggerNode("party member medium health", { NextAction("healing wave on party", 26.0f) }));
    triggers.push_back(new TriggerNode("party member almost full health", { NextAction("lesser healing wave on party", 24.0f) }));
