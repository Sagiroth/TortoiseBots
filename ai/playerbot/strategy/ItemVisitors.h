...
namespace ai
{
    ...
    {
    public:
        FindItemUsageVisitor(Player* bot, ItemUsage usage = ItemUsage::ITEM_USAGE_NONE) : FindItemVisitor(), bot(bot), usage(usage) { context = PlayerbotAIStorage::Instance().GetAI(bot)->GetAiObjectContext();};
    ...
    {
    public:
        FindVendorItemsVisitor(Player* bot, bool includeAH) : FindItemVisitor(), bot(bot), includeAH(includeAH) { context = PlayerbotAIStorage::Instance().GetAI(bot)->GetAiObjectContext(); };
