...
* Queue in the LFT tool with your own managed bots in the party (alts/hires): the core auto-answers the rolecheck for them from `AiFactory::GetPlayerRoles` — your explicit `.bot role <Name> <tank|healer|dps|clear>` (or `role self`, same override) wins, otherwise automatic spec/gear detection applies. The TBM party-frame Tank button sends that same `.bot role` command; its healer/DPS buttons send strategy commands (kit change only, role answer stays on auto-detect).
