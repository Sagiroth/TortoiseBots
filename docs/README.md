...
| **Enable an active player economy on the Auction House** | Set `AiPlayerbot.AhMarketEnabled = 1` → [Living AH Economy](guides/living-world.md#5-the-living-auction-house-economy-ahmarketservice) |
...
| **Autofill missing roles for LFT dungeon runs** | On by default (`AiPlayerbot.RandomBotLftEnabled = 1`); set `0` to opt out. Your own party bots accept the offer automatically — `.bot summon` stragglers to the portal → [LFT Autofill](guides/living-world.md#6-automated-dungeon--battleground-queues) |
| **Rebuild the bot pool after changing bot gear, skills or professions** | Set `AiPlayerbot.RandomBotPoolReset = once:<new-token>` (and `RandomBotAutoCreate = 1`), restart, then watch `bot pool status` → [Resetting the managed bot pool](guides/living-world.md#8-resetting-the-managed-bot-pool) |
| **Enroll my existing hand-made `RNDBOT*` accounts (one-time upgrade)** | At the server console: `bot pool adopt preview`, then `bot pool adopt confirm <challenge>` → [Pool Administration](guides/player-controls.md#7-managed-pool-administration-bot-pool) |
| **Inspect stuck bots and live positions on a 2D map** | Run Go daemon in `tools/observability` and open `http://localhost:8095` (the default port; `--http-port` changes it) → [Observability Dashboard](guides/observability-dashboard.md) |
