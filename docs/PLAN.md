...
* **BG auto-queue:** `BattlegroundQueueService` is default-off and demand-aware for WSG/AB/AV, uses the copy-only generic demand snapshot from core PR #415 plus native join/leave handlers, matches queue type/bracket and underrepresented faction, tracks service-owned queue pairs, and reconciles master reclaim without queue-internal access.
