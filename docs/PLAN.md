...
* **Commands:** `.bot add/remove/logout/roster/action` now provide the Actions/Roster control plane; legacy `.bot follow/invite/uninvite/stay/guard/free/ready/attack/interrupt/formation/list/stats/status/pullback/summon/command/help` remains account/GM-gated and compatible. `.bot command` still delegates to `PlayerbotAI::HandleCommand`; `.bot action` resolves target scope/executors server-side and emits structured `TBM:` results. `interrupt` selects a ready mature class/pet interrupt for the current cast target and queues existing reach movement when necessary.
